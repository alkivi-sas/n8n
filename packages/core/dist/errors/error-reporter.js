...
let ErrorReporter = class ErrorReporter {
	...
	async init({ beforeSendFilter, dsn, serverType, release, environment, serverName, releaseDate }) {
		...
		const { init, captureException, setTag } = await Promise.resolve().then(() =>
			__importStar(require('@sentry/node')),
		);
		const { requestDataIntegration, rewriteFramesIntegration } = await Promise.resolve().then(() =>
			__importStar(require('@sentry/node')),
		);
	...
	async beforeSend(event, hint) {
		...
		if (originalException instanceof axios_1.AxiosError) return null;
		...
		if (
			originalException instanceof n8n_workflow_1.ApplicationError ||
			originalException instanceof n8n_workflow_1.BaseError
		) {
		...
		if (originalException instanceof Error && originalException.stack) {
			const eventHash = (0, node_crypto_1.createHash)('sha1')
				.update(originalException.stack)
				.digest('base64');
	...
	error(e, options) {
		if (e instanceof n8n_workflow_1.ExecutionCancelledError) return;
	...
	wrap(e) {
		...
		if (typeof e === 'string') return new n8n_workflow_1.ApplicationError(e);
		return;
	}
...
exports.ErrorReporter = ErrorReporter = __decorate(
	[(0, di_1.Service)(), __metadata('design:paramtypes', [logger_1.Logger])],
	ErrorReporter,
);
