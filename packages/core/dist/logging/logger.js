...
let Logger = (Logger_1 = class Logger {
	...
	constructor(globalConfig, instanceSettingsConfig, { isRoot } = { isRoot: true }) {
		...
		if (isRoot) n8n_workflow_1.LoggerProxy.init(this);
	}
	...
	setConsoleTransport() {
		const format =
			this.level === 'debug' && constants_1.inDevelopment
	...
	debugDevConsoleFormat() {
		return winston_1.default.format.combine(
			...
			winston_1.default.format.timestamp({ format: () => this.devTsFormat() }),
			this.color(),
			this.scopeFilter(),
			winston_1.default.format.printf(
				({ level: rawLevel, message, timestamp, metadata: rawMetadata }) => {
					...
					return [timestamp, level, message + ' ' + picocolors_1.default.dim(metadata)].join(
						separator,
					);
				},
			),
	...
	debugProdConsoleFormat() {
		return winston_1.default.format.combine(
			...
			this.color(),
			this.scopeFilter(),
			winston_1.default.format.printf(({ level, message, timestamp, metadata: rawMetadata }) => {
				const metadata = this.toPrintable(rawMetadata);
	...
	setFileTransport() {
		...
		const filename = node_path_1.default.join(
		...
		__decorate(
			[
				...
				__metadata('design:paramtypes', [
					...
					instance_settings_config_1.InstanceSettingsConfig,
					Object,
				]),
			],
			Logger,
