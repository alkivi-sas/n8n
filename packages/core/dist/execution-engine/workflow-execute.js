...
class WorkflowExecute {
	...
	runPartialWorkflow2(workflow, runData, pinData = {}, dirtyNodeNames = [], destinationNodeName) {
		...
		if (destinationHasNoParents) {
			...
			const { nodeExecutionStack, waitingExecution, waitingExecutionSource } = (0,
			partial_execution_utils_1.recreateNodeExecutionStack)(
				graph,
				new Set([destination]),
				runData,
				pinData ?? {},
		...
		const trigger = (0, partial_execution_utils_1.findTriggerForPartialExecution)(
			workflow,
			destinationNodeName,
		);
		...
		let startNodes = (0, partial_execution_utils_1.findStartNodes)({
			graph,
			trigger,
			destination,
			runData,
			...
			for (const nodeName of Object.keys(metadata)) {
				for ([index, metaRunData] of metadata[nodeName].entries()) {
					...
					if (taskData) {
						...
						di_1.Container.get(error_reporter_1.ErrorReporter).error(
							new n8n_workflow_1.UnexpectedError('Taskdata missing at the end of an execution'),
	...
	addNodeToBeExecuted(
		...
		let connectionDataArray = (0, get_1.default)(
			this.runExecutionData,
			[
				'executionData',
				'waitingExecution',
	...
	checkReadyForExecution(workflow, inputData = {}) {
		...
		for (const nodeName of checkNodes) {
			...
			if (nodeType === undefined) {
				...
				nodeIssues = n8n_workflow_1.NodeHelpers.getNodeParametersIssues(
	...
	async runNode(
		...
		if (nodeType.execute || customOperation) {
			...
			const context = new node_execution_context_1.ExecuteContext(
				workflow,
				node,
				additionalData,
				mode,
			...
			if (mode === 'manual') {
				const context = new node_execution_context_1.PollContext(
					workflow,
					node,
					additionalData,
					mode,
			...
			if (mode === 'manual') {
				const triggerResponse = await di_1.Container.get(
				...
				executionLoop: while (this.runExecutionData.executionData.nodeExecutionStack.length !== 0) {
					...
					for (let tryIndex = 0; tryIndex < maxTries; tryIndex++) {
						try {
							...
							n8n_workflow_1.LoggerProxy.debug(
								`Running node "${executionNode.name}" finished successfully`,
								{
									node: executionNode.name,
									workflowId: workflow.id,
								},
							...
							n8n_workflow_1.LoggerProxy.debug(
								`Running node "${executionNode.name}" finished with error`,
								{
									node: executionNode.name,
									workflowId: workflow.id,
								},
					...
					if (
						...
						for (let i = 0; i < waitingNodes.length; i++) {
							...
							const inputsWithData = this.runExecutionData.executionData.waitingExecution[nodeName][
								firstRunIndex
							].main
				...
				.then(async () => {
					if (this.status === 'canceled' && executionError === undefined) {
						return await this.processSuccessExecution(
							startedAt,
							workflow,
							new n8n_workflow_1.ExecutionCancelledError(
	...
	async processSuccessExecution(startedAt, workflow, executionError, closeFunction) {
		...
		if (executionError !== undefined) {
			...
			n8n_workflow_1.LoggerProxy.debug(
				`Workflow execution will wait until ${this.runExecutionData.waitTill}`,
				{
					workflowId: workflow.id,
				},
			);
			...
			n8n_workflow_1.LoggerProxy.debug('Workflow execution finished successfully', {
				workflowId: workflow.id,
			});
	...
	handleNodeErrorOutput(workflow, executionData, nodeSuccessData, runIndex) {
		...
		const outputs = n8n_workflow_1.NodeHelpers.getNodeOutputs(
			workflow,
			executionData.node,
			nodeType.description,
		);
		...
		const mainOutputTypes = outputTypes.filter(
			(output) => output === n8n_workflow_1.NodeConnectionTypes.Main,
		);
		const errorItems = [];
		const closeFunctions = [];
		const executeFunctions = new node_execution_context_1.ExecuteContext(
			workflow,
			executionData.node,
			this.additionalData,
			this.mode,
			...
			while (items.length) {
				...
				if (errorData) {
					...
					if (executionData.source === null || pairedItemData === undefined) {
						...
						const sourceData =
							executionData.source[n8n_workflow_1.NodeConnectionTypes.Main][pairedItemInputIndex];
