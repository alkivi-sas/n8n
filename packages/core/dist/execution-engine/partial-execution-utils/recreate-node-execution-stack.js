...
function recreateNodeExecutionStack(graph, startNodes, runData, pinData) {
	...
	for (const startNode of startNodes) {
		...
		if (incomingStartNodeConnections.length === 0) {
			...
			const sourceDataSets = (0, get_source_data_groups_1.getSourceDataGroups)(
				graph,
				startNode,
				runData,
				pinData,
				...
				if (sourceData.complete) {
					...
					for (const incomingConnection of sourceData.connections) {
						...
						if (pinData[sourceNode.name]) {
							...
							const nodeIncomingData = (0, get_incoming_data_1.getIncomingDataFromAnyRun)(
								runData,
								sourceNode.name,
					...
					for (const incomingConnection of sourceData.connections) {
						const sourceNode = incomingConnection.from;
						const maybeNodeIncomingData = (0, get_incoming_data_1.getIncomingDataFromAnyRun)(
							runData,
							sourceNode.name,
							incomingConnection.type,
							incomingConnection.outputIndex,
