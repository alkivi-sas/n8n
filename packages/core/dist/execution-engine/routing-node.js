...
class RoutingNode {
	...
	async runNode() {
		...
		if (credentialsDecrypted) {
			...
			try {
				credentials = (await context.getCredentials(credentialDescription.name, 0)) || {};
			} catch (error) {
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			...
			const { proxy, timeout, allowUnauthorizedCerts } = itemContext[
				itemIndex
			].thisArgs.getNodeParameter('requestOptions', 0, {});
			...
			if (proxy) {
				...
				for (const property of proxyProperties) {
					if (!(property in proxyParsed) || proxyParsed[property] === null) {
				...
				if (requestOperations.pagination.type === 'generic') {
					...
					do {
						...
						paginateRequestData = this.getParameterValue(
							requestOperations.pagination.properties.request,
							itemIndex,
							runIndex,
							executeSingleFunctions.getExecuteData(),
							additionalKeys,
					...
					requestData.options[optionsType][properties.limitParameter] = properties.pageSize;
					...
					do {
						if (requestData?.maxResults) {
							...
							requestData.options[optionsType][properties.limitParameter] = Math.min(
								properties.pageSize,
								resultsMissing,
							);
						}
						...
						if (properties.rootProperty) {
							const tempResponseValue = (0, get_1.default)(
	...
	getRequestOptionsFromParameters(
		...
		if (
			!n8n_workflow_1.NodeHelpers.displayParameter(
				node.parameters,
				nodeProperties,
				node,
		...
		if (nodeProperties.type === 'options') {
			...
			const selectedOption = nodeProperties.options.filter(
			...
			for (const propertyOption of nodeProperties.options) {
				if (Object.keys(value).includes(propertyOption.name) && propertyOption.type !== undefined) {
					const tempOptions = this.getRequestOptionsFromParameters(
						executeSingleFunctions,
						propertyOption,
						itemIndex,
