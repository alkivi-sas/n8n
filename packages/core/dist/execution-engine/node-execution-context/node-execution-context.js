...
class NodeExecutionContext {
	...
	get nodeInputs() {
		return n8n_workflow_1.NodeHelpers.getNodeInputs(
			this.workflow,
			this.node,
	...
	get nodeOutputs() {
		return n8n_workflow_1.NodeHelpers.getNodeOutputs(
			this.workflow,
			this.node,
			this.nodeType.description,
	...
	get additionalKeys() {
		return (0, get_additional_keys_1.getAdditionalKeys)(
			this.additionalData,
			this.mode,
	...
	_getNodeParameter(parameterName, itemIndex, fallbackValue, options) {
		...
		if (value === undefined) {
			throw new n8n_workflow_1.ApplicationError('Could not get parameter', {
				extra: { parameterName },
			});
		}
		...
		try {
			...
			if (
				e instanceof n8n_workflow_1.ExpressionError &&
				node.continueOnFail &&
		...
		if (options?.extractValue) {
			returnData = (0, extract_value_1.extractValue)(
				returnData,
				parameterName,
				node,
				nodeType,
		...
		returnData = (0, validate_value_against_schema_1.validateValueAgainstSchema)(
			node,
			nodeType,
			returnData,
			parameterName,
...
__decorate(
	[decorators_1.Memoized, __metadata('design:type', Object), __metadata('design:paramtypes', [])],
	NodeExecutionContext.prototype,
	'logger',
	null,
);
__decorate(
