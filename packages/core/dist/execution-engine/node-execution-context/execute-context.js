...
class ExecuteContext extends base_execute_context_1.BaseExecuteContext {
	constructor(
		...
		this.helpers = {
			...
			...(0, request_helper_functions_1.getRequestHelperFunctions)(
				workflow,
				node,
				additionalData,
				runExecutionData,
			...
			assertBinaryData: (itemIndex, propertyName) =>
				(0, binary_helper_functions_1.assertBinaryData)(
					inputData,
					node,
					itemIndex,
					propertyName,
			...
			getBinaryDataBuffer: async (itemIndex, propertyName) =>
				await (0, binary_helper_functions_1.getBinaryDataBuffer)(
					inputData,
					itemIndex,
					propertyName,
					0,
		...
		this.nodeHelpers = {
			copyBinaryFile: async (filePath, fileName, mimeType) =>
				await (0, binary_helper_functions_1.copyBinaryFile)(
					this.workflow.id,
	...
	async startJob(jobType, settings, itemIndex) {
		return await this.additionalData.startRunnerTask(
			this.additionalData,
			jobType,
			settings,
			this,
			...
			{},
			this.mode,
			(0, n8n_workflow_1.createEnvProviderState)(),
			this.executeData,
		);
	}
	async getInputConnectionData(connectionType, itemIndex) {
		return await get_input_connection_data_1.getInputConnectionData.call(
			this,
			this.workflow,
			this.runExecutionData,
			this.runIndex,
	...
	addOutputData() {
		throw new n8n_workflow_1.ApplicationError(
			'addOutputData should not be called on IExecuteFunctions',
		);
	}
