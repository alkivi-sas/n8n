...
function digestAuthAxiosConfig(axiosConfig, response, auth) {
	const authDetails = response.headers['www-authenticate'].split(',').map((v) => v.split('='));
	if (authDetails) {
		...
		const response = crypto_1.default
			...
			.digest('hex');
		let authorization =
			`Digest username="${auth?.username}",realm="${realm}",` +
...
async function parseRequestObject(requestObject) {
	...
	const contentType = contentTypeHeaderKeyName && axiosConfig.headers?.[contentTypeHeaderKeyName];
...
async function proxyRequestToAxios(workflow, additionalData, node, uriOrObject, options) {
	...
	try {
		...
		if (error.isAxiosError) {
			...
			if (response) {
				di_1.Container.get(logger_1.Logger).debug('Request proxied to Axios failed', {
...
function convertN8nRequestToAxios(n8nRequest) {
	...
	if (body) {
		...
		if (
			typeof body === 'string' ||
			(typeof body === 'object' && !(0, n8n_workflow_1.isObjectEmpty)(body))
		) {
...
async function requestOAuth2(
	...
	const accessToken =
		(0, get_1.default)(oauthTokenData, oAuth2Options?.property) || oauthTokenData.accessToken;
...
async function requestWithAuthentication(
	...
	try {
		...
		try {
			...
			if (error instanceof n8n_workflow_1.ExecutionBaseError) throw error;
			throw new n8n_workflow_1.NodeApiError(this.getNode(), error);
		}
	}
}
const getRequestHelperFunctions = (
	...
	async function requestWithAuthenticationPaginated(
		...
		do {
			...
			let contentBody;
			if (
				newResponse.body instanceof stream_1.Readable &&
