...
function isFilePathBlocked(filePath) {
	...
	const blockFileAccessToN8nFiles =
		process.env[constants_1.BLOCK_FILE_ACCESS_TO_N8N_FILES] !== 'false';
...
const getFileSystemHelperFunctions = (node) => ({
	async createReadStream(filePath) {
		...
		if (isFilePathBlocked(filePath)) {
			...
			throw new n8n_workflow_1.NodeOperationError(
				node,
				`Access to the file is not allowed.${message}`,
				{
					level: 'warning',
				},
			);
	...
	getStoragePath() {
		return (0, node_path_1.join)(
			di_1.Container.get(instance_settings_1.InstanceSettings).n8nFolder,
			`storage/${node.type}`,
		);
	},
	async writeContentToFile(filePath, content, flag) {
		if (isFilePathBlocked(filePath)) {
			throw new n8n_workflow_1.NodeOperationError(
				node,
				`The file "${String(filePath)}" is not writable.`,
				{
					level: 'warning',
				},
			);
