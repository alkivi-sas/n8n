...
const validateResourceMapperValue = (
	...
	for (let i = 0; i < paramValueNames.length; i++) {
		...
		if (schemaEntry?.type) {
			const validationResult = (0, n8n_workflow_1.validateFieldType)(
				key,
				resolvedValue,
				schemaEntry.type,
				{
...
const validateCollection = (
	...
	if (propertyDescription.type === 'fixedCollection') {
		nestedDescriptions = propertyDescription.options.find(
	...
	if (validationResult.valid) {
		for (const value of Array.isArray(validationResult.newValue)
			...
			for (const key of Object.keys(value)) {
				...
				const fieldValidationResult = (0, n8n_workflow_1.validateFieldType)(
					key,
					value[key],
				...
				if (!fieldValidationResult.valid) {
					throw new n8n_workflow_1.ExpressionError(
...
const validateValueAgainstSchema = (
	...
	if (
		...
		validationResult = (0, n8n_workflow_1.validateFieldType)(
			parameterName,
			parameterValue,
			propertyDescription.validateType,
		);
