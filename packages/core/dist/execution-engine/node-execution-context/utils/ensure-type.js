...
function ensureType(toType, parameterValue, parameterName, errorOptions) {
		...
		if (typeof returnData !== 'object') {
			if (typeof returnData === 'string' && returnData.length) {
				try {
					...
					throw new n8n_workflow_1.ExpressionError(
						`Parameter '${parameterName}' could not be parsed`,
						{
							...errorOptions,
	...
	try {
		...
		if (error instanceof n8n_workflow_1.ExpressionError) throw error;
		throw new n8n_workflow_1.ExpressionError(
			`Parameter '${parameterName}' could not be converted to ${toType}`,
			{
				...errorOptions,
				description: error.message,
			},
