...
async function checkProcessedAndRecord(items, scope, contextData, options) {
	return await data_deduplication_service_1.DataDeduplicationService.getInstance().checkProcessedAndRecord(
		items,
		scope,
		contextData,
		options,
...
async function checkProcessedItemsAndRecord(key, items, scope, contextData, options) {
	return await data_deduplication_service_1.DataDeduplicationService.getInstance().checkProcessedItemsAndRecord(
		key,
		items,
		scope,
		contextData,
...
async function removeProcessed(items, scope, contextData, options) {
	return await data_deduplication_service_1.DataDeduplicationService.getInstance().removeProcessed(
		items,
		scope,
		contextData,
		options,
...
async function clearAllProcessedItems(scope, contextData, options) {
	return await data_deduplication_service_1.DataDeduplicationService.getInstance().clearAllProcessedItems(
		scope,
		contextData,
		options,
	);
...
async function getProcessedDataCount(scope, contextData, options) {
	return await data_deduplication_service_1.DataDeduplicationService.getInstance().getProcessedDataCount(
		scope,
		contextData,
		options,
	);
