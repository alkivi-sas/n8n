...
async function getBinaryMetadata(binaryDataId) {
	return await di_1.Container.get(binary_data_service_1.BinaryDataService).getMetadata(
		binaryDataId,
	);
}
async function getBinaryStream(binaryDataId, chunkSize) {
	return await di_1.Container.get(binary_data_service_1.BinaryDataService).getAsStream(
		binaryDataId,
		chunkSize,
	);
}
...
async function setBinaryDataBuffer(binaryData, bufferOrStream, workflowId, executionId) {
	return await di_1.Container.get(binary_data_service_1.BinaryDataService).store(
		workflowId,
		executionId,
		bufferOrStream,
		binaryData,
...
async function copyBinaryFile(workflowId, executionId, filePath, fileName, mimeType) {
	...
	return await di_1.Container.get(binary_data_service_1.BinaryDataService).copyBinaryFile(
		workflowId,
		executionId,
		returnData,
		filePath,
...
const getBinaryHelperFunctions = ({ executionId }, workflowId) => ({
	...
	copyBinaryFile: async () => {
		throw new n8n_workflow_1.ApplicationError(
			'`copyBinaryFile` has been removed. Please upgrade this node.',
		);
	},
});
