...
async function getInputConnectionData(
	...
	for (const connectedNode of connectedNodes) {
		...
		const contextFactory = (runIndex, inputData) =>
			new supply_data_context_1.SupplyDataContext(
				workflow,
				connectedNode,
				additionalData,
				mode,
		...
		if (!connectedNodeType.supplyData) {
			if (
				connectedNodeType.description.outputs.includes(n8n_workflow_1.NodeConnectionTypes.AiTool)
			) {
