...
function findPropertyFromParameterName(parameterName, nodeType, node, nodeParameters) {
	...
	const findProp = (name, options) => {
		return options.find(
			(i) =>
				i.name === name &&
				n8n_workflow_1.NodeHelpers.displayParameterPath(
					nodeParameters,
					i,
					currentParamPath,
					node,
	...
	for (const p of paramParts) {
		...
		if (!property) {
			...
			throw new n8n_workflow_1.ApplicationError('Could not find property', {
				extra: { parameterName },
			});
		}
		if (!property) {
...
function extractValue(value, parameterName, node, nodeType, itemIndex = 0) {
	...
	try {
		...
		throw new n8n_workflow_1.NodeOperationError(node, error, {
			description: (0, get_1.default)(error, 'description'),
		});
	}
}
