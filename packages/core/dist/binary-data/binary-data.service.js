...
let BinaryDataService = class BinaryDataService {
	...
	async init(config) {
		...
		if (config.availableModes.includes('filesystem')) {
			const { FileSystemManager } = await Promise.resolve().then(() =>
				__importStar(require('./file-system.manager')),
			);
		...
		if (config.availableModes.includes('s3')) {
			const { ObjectStoreManager } = await Promise.resolve().then(() =>
				__importStar(require('./object-store.manager')),
			);
			const { ObjectStoreService } = await Promise.resolve().then(() =>
				__importStar(require('./object-store/object-store.service.ee')),
			);
