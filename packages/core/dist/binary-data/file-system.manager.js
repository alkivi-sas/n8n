...
class FileSystemManager {
	...
	async getMetadata(fileId) {
		...
		return await (0, n8n_workflow_1.jsonParse)(
			await promises_1.default.readFile(filePath, { encoding: 'utf-8' }),
		);
	}
	async deleteMany(ids) {
		...
		for (const fileName of fileNames) {
			...
			if (executionId && set.has(executionId)) {
				...
				await Promise.all([
					promises_1.default.rm(filePath),
					promises_1.default.rm(`${filePath}.metadata`),
				]);
			}
		}
	...
	toFileId(workflowId, executionId) {
		if (!executionId) executionId = 'temp';
