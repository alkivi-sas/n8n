...
let InstanceSettings = class InstanceSettings {
	...
	loadOrCreate() {
		...
		const encryptionKey =
			process.env.N8N_ENCRYPTION_KEY ?? (0, crypto_1.randomBytes)(24).toString('base64');
	...
	ensureSettingsFilePermissions() {
		...
		if (this.enforceSettingsFilePermissions.enforce) {
			...
			const chmodResult = (0, n8n_workflow_1.toResult)(() =>
				(0, node_fs_1.chmodSync)(this.settingsFile, 0o600),
			);
			if (!chmodResult.ok) {
				this.logger.warn(
