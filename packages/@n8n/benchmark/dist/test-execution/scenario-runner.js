...
class ScenarioRunner {
	...
	async runManyScenarios(scenarios) {
		...
		const authenticatedN8nClient =
			await authenticated_n8n_api_client_1.AuthenticatedN8nApiClient.createUsingUsernameAndPassword(
				this.n8nClient,
				this.ownerConfig,
			);
		const testDataImporter = new scenario_data_importer_1.ScenarioDataImporter(
			authenticatedN8nClient,
		);
