...
class JsTaskRunner extends task_runner_1.TaskRunner {
	constructor(config, name = 'JS Task Runner') {
		...
		this.taskDataReconstruct =
			new data_request_response_reconstruct_1.DataRequestResponseReconstruct();
		const { jsRunnerConfig } = config;
		const parseModuleAllowList = (moduleList) =>
			moduleList === '*'
				? '*'
				: new Set(
						moduleList
	...
	preventPrototypePollution(allowedExternalModules) {
		...
		[
			n8n_workflow_1.Workflow,
			n8n_workflow_1.Expression,
			n8n_workflow_1.WorkflowDataProxy,
			luxon_1.DateTime,
			luxon_1.Interval,
			luxon_1.Duration,
		]
	...
	async executeTask(taskParams, abortSignal) {
		...
		const dataResponse = await this.requestData(
			taskId,
	...
	async runForAllItems(taskId, settings, data, workflow, signal) {
		...
		try {
			const result = await new Promise((resolve, reject) => {
				...
				const taskResult = (0, node_vm_1.runInContext)(
	...
	createDataProxy(data, workflow, itemIndex) {
		return new n8n_workflow_1.WorkflowDataProxy(
			workflow,
			...
			data.mode,
			(0, n8n_core_1.getAdditionalKeys)(data.additionalData, data.mode, data.runExecutionData),
			data.executeData,
	...
	buildRpcCallObject(taskId) {
		...
		for (const rpcMethod of runner_types_1.EXPOSED_RPC_METHODS) {
			(0, set_1.default)(
				rpcObject,
				rpcMethod.split('.'),
				async (...args) => await this.makeRpcCall(taskId, rpcMethod, args),
			);
		}
