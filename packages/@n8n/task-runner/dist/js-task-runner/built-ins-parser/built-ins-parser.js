	...
	handlePrevNodeCall(_node, state, ancestors) {
		...
		if ((0, acorn_helpers_1.isMemberExpression)(directParent)) {
			...
			if (directParent.computed) {
				...
				(0, acorn_helpers_1.isIdentifier)(accessedProperty) &&
				this.isPairedItemProperty(accessedProperty.name)
			) {
				state.markNeedsAllNodes();
			}
		} else if (
			(0, acorn_helpers_1.isVariableDeclarator)(directParent) ||
			(0, acorn_helpers_1.isAssignmentExpression)(directParent)
		) {
			state.markNeedsAllNodes();
		} else {
			state.markNeedsAllNodes();
