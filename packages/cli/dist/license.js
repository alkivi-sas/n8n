...
let License = class License {
	...
	getUsersLimit() {
		return (
			this.getFeatureValue(constants_1.LICENSE_QUOTAS.USERS_LIMIT) ??
			constants_1.UNLIMITED_LICENSE_QUOTA
		);
	}
	getTriggerLimit() {
		return (
			this.getFeatureValue(constants_1.LICENSE_QUOTAS.TRIGGER_LIMIT) ??
			constants_1.UNLIMITED_LICENSE_QUOTA
		);
	}
	getVariablesLimit() {
		return (
			this.getFeatureValue(constants_1.LICENSE_QUOTAS.VARIABLES_LIMIT) ??
			constants_1.UNLIMITED_LICENSE_QUOTA
		);
	}
	getAiCredits() {
	...
	getWorkflowHistoryPruneLimit() {
		return (
			this.getFeatureValue(constants_1.LICENSE_QUOTAS.WORKFLOW_HISTORY_PRUNE_LIMIT) ??
			constants_1.UNLIMITED_LICENSE_QUOTA
		);
	}
	...
	getInsightsRetentionPruneInterval() {
		return (
			this.getFeatureValue(constants_1.LICENSE_QUOTAS.INSIGHTS_RETENTION_PRUNE_INTERVAL_DAYS) ?? 24
		);
	}
	getTeamProjectLimit() {
	...
	checkIsLicensedForMultiMain(features) {
		...
		const isMultiMainLicensed =
			features[constants_1.LICENSE_FEATURES.MULTIPLE_MAIN_INSTANCES] ?? false;
