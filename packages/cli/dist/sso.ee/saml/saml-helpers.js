...
async function setSamlLoginEnabled(enabled) {
	if (
		...
		throw new internal_server_error_1.InternalServerError(
			`Cannot switch SAML login enabled state when an authentication method other than email or saml is active (current: ${(0, sso_helpers_1.getCurrentAuthenticationMethod)()})`,
		);
	}
}
...
async function createUserFromSamlAttributes(attributes) {
	...
	return await userRepository.manager.transaction(async (trx) => {
		...
		await trx.save(
			trx.create(auth_identity_1.AuthIdentity, {
...
async function updateUserFromSamlAttributes(user, attributes) {
	...
	await di_1.Container.get(auth_identity_repository_1.AuthIdentityRepository).save(
		samlAuthIdentity,
		{ transaction: false },
	);
	user.firstName = attributes.firstName;
	user.lastName = attributes.lastName;
	const resultUser = await di_1.Container.get(user_repository_1.UserRepository).save(user, {
		transaction: false,
	});
