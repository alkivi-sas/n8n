...
let TestDefinitionsController = class TestDefinitionsController {
	...
	async getMany(req) {
		const userAccessibleWorkflowIds = await (0, workflows_service_1.getSharedWorkflowIds)(
			req.user,
			['workflow:read'],
		);
	...
	async getOne(req) {
		...
		if (!testDefinition) throw new not_found_error_1.NotFoundError('Test definition not found');
		return testDefinition;
	}
	async create(req, res) {
		const bodyParseResult =
			test_definition_schema_1.testDefinitionCreateRequestBodySchema.safeParse(req.body);
		...
		if (
			...
			throw new forbidden_error_1.ForbiddenError(
				'User does not have access to the evaluation workflow',
			);
		}
	...
	async patch(req, res) {
		const { id: testDefinitionId } = req.params;
		const bodyParseResult = test_definition_schema_1.testDefinitionPatchRequestBodySchema.safeParse(
			req.body,
		);
		if (!bodyParseResult.success) {
			res.status(400).json({ errors: bodyParseResult.error.errors });
		...
		if (!existingTest) throw new not_found_error_1.NotFoundError('Test definition not found');
		if (
			req.body.evaluationWorkflowId &&
			!userAccessibleWorkflowIds.includes(req.body.evaluationWorkflowId)
		) {
	...
	async runTest(req, res) {
		const { id: testDefinitionId } = req.params;
		const workflowIds = await (0, workflows_service_1.getSharedWorkflowIds)(req.user, [
			'workflow:read',
		]);
...
__decorate(
	...
	TestDefinitionsController.prototype,
	'getMany',
	null,
);
__decorate(
...
exports.TestDefinitionsController = TestDefinitionsController = __decorate(
	[
		...
		__metadata('design:paramtypes', [
			test_definition_service_ee_1.TestDefinitionService,
