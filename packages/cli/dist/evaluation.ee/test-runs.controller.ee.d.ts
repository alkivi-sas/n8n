...
export declare class TestRunsController {
	...
	getMany(req: TestRunsRequest.GetMany): Promise<
		{
			finalResult:
				| import('../databases/repositories/test-run.repository.ee').TestRunFinalResult
				| null;
	...
	>;
	getOne(
		req: TestRunsRequest.GetOne,
	): Promise<import('../databases/repositories/test-run.repository.ee').TestRunSummary>;
	getTestCases(
		req: TestRunsRequest.GetCases,
	): Promise<import('../databases/entities/test-case-execution.ee').TestCaseExecution[]>;
