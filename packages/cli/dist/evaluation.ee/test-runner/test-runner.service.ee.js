...
let TestRunnerService = (TestRunnerService_1 = class TestRunnerService {
	...
	getStartNodesData(workflow, pastExecutionData, pastExecutionWorkflowData) {
		...
		(0, node_assert_1.default)(
			...
			'Could not find the trigger node of the past execution',
		);
		const pastExecutionTriggerNodeId = pastWorkflowNodeIdByName.get(pastExecutionTriggerNode);
		(0, node_assert_1.default)(
			pastExecutionTriggerNodeId,
			'Could not find the trigger node ID of the past execution',
		);
	...
	async runTestCase(
		...
		const pinData = (0, utils_ee_1.createPinData)(
			workflow,
			mockedNodes,
			pastExecutionData,
			pastExecutionWorkflowData,
	...
	async syncMetrics(testDefinitionId, evaluationWorkflow) {
		...
		const metricsToAdd = (0, difference_1.default)(
			Array.from(usedTestMetricNames),
			Array.from(existingMetricNames),
		);
		const metricsToRemove = (0, difference_1.default)(
			Array.from(existingMetricNames),
			Array.from(usedTestMetricNames),
		);
	...
	async runTestCaseEvaluation(evaluationWorkflow, evaluationInputData, abortSignal, metadata) {
		...
		const data = await (0, workflow_execute_additional_data_1.getRunData)(evaluationWorkflow, [
			evaluationInputData,
		]);
	...
	extractEvaluationResult(execution, evaluationWorkflow) {
		...
		(0, node_assert_1.default)(
			lastNodeExecuted,
			'Could not find the last node executed in evaluation workflow',
		);
		...
		try {
			...
			for (const pastExecutionId of pastExecutionIds) {
				...
				try {
					...
					const evaluationInputData = (0, utils_ee_1.formatTestCaseExecutionInputData)(
						originalRunData,
						pastExecution.executionData.workflowData,
						testCaseRunData,
						workflow,
