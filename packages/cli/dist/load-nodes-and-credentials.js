...
let LoadNodesAndCredentials = class LoadNodesAndCredentials {
	...
	async init() {
		if (constants_1.inTest) throw new n8n_workflow_1.UnexpectedError('Not available in tests');
	...
	async setupHotReload() {
		...
		Object.values(this.loaders).forEach(async (loader) => {
			...
			const realModulePath = path_1.default.join(
				await promises_1.default.realpath(loader.directory),
				path_1.default.sep,
			);
