...
let SourceControlService = class SourceControlService {
	...
	async pushWorkfolder(options) {
		...
		this.eventService.emit(
			...
			(0, source_control_helper_ee_1.getTrackingInformationFromPostPushResult)(statusResult),
		);
		return {
			statusCode: 200,
			pushResult,
	...
	async pullWorkfolder(user, options) {
		...
		this.eventService.emit(
			...
			(0, source_control_helper_ee_1.getTrackingInformationFromPullResult)(statusResult),
		);
		return {
			statusCode: 200,
			statusResult,
	...
	async getStatus(options) {
		...
		if (options.direction === 'push') {
			...
			this.eventService.emit(
				...
				(0, source_control_helper_ee_1.getTrackingInformationFromPullResult)(sourceControlledFiles),
			);
		}
		if (options?.verbose) {
			return {
