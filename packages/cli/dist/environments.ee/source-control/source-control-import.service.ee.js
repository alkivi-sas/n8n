...
let SourceControlImportService = class SourceControlImportService {
	...
	async getRemoteVersionIdsFromFiles() {
		...
		const remoteWorkflowFilesParsed = await Promise.all(
			remoteWorkflowFiles.map(async (file) => {
				...
				const remote = (0, n8n_workflow_1.jsonParse)(
				...
				return {
					...
					remoteId: remote.id,
					filename: (0, source_control_helper_ee_1.getWorkflowExportPath)(
						remote.id,
	...
	async getRemoteCredentialsFromFiles() {
		...
		const remoteCredentialFilesParsed = await Promise.all(
			remoteCredentialFiles.map(async (file) => {
				...
				return {
					...remote,
					filename: (0, source_control_helper_ee_1.getCredentialExportPath)(
						remote.id,
	...
	async getRemoteVariablesFromFile() {
		const variablesFile = await (0, fast_glob_1.default)(
		...
		if (variablesFile.length > 0) {
			...
			return (0, n8n_workflow_1.jsonParse)(
				await (0, promises_1.readFile)(variablesFile[0], { encoding: 'utf8' }),
				{
					fallbackValue: [],
				},
			);
	...
	async getRemoteFoldersAndMappingsFromFile() {
		const foldersFile = await (0, fast_glob_1.default)(
		...
		if (foldersFile.length > 0) {
			...
			const mappedFolders = (0, n8n_workflow_1.jsonParse)(
				await (0, promises_1.readFile)(foldersFile[0], { encoding: 'utf8' }),
				{
	...
	async getRemoteTagsAndMappingsFromFile() {
		...
		if (tagsFile.length > 0) {
			...
			const mappedTags = (0, n8n_workflow_1.jsonParse)(
				await (0, promises_1.readFile)(tagsFile[0], { encoding: 'utf8' }),
	...
	async importWorkflowFromWorkFolder(candidates, userId) {
		...
		for (const candidate of candidates) {
			...
			const importedWorkflow = (0, n8n_workflow_1.jsonParse)(
				await (0, promises_1.readFile)(candidate.file, { encoding: 'utf8' }),
			);
	...
	async importCredentialsFromWorkFolder(candidates, userId) {
		...
		importCredentialsResult = await Promise.all(
			candidates.map(async (candidate) => {
				...
				const credential = (0, n8n_workflow_1.jsonParse)(
					await (0, promises_1.readFile)(candidate.file, { encoding: 'utf8' }),
				);
	...
	async importTagsFromWorkFolder(candidate) {
		...
		try {
			...
			mappedTags = (0, n8n_workflow_1.jsonParse)(
				await (0, promises_1.readFile)(candidate.file, { encoding: 'utf8' }),
				{ fallbackValue: { tags: [], mappings: [] } },
			);
		} catch (e) {
	...
	async importFoldersFromWorkFolder(user, candidate) {
		...
		try {
			...
			mappedFolders = (0, n8n_workflow_1.jsonParse)(
				await (0, promises_1.readFile)(candidate.file, { encoding: 'utf8' }),
				{
					fallbackValue: { folders: [] },
				},
	...
	async importVariablesFromWorkFolder(candidate, valueOverrides) {
		...
		try {
			...
			importedVariables = (0, n8n_workflow_1.jsonParse)(
				await (0, promises_1.readFile)(candidate.file, { encoding: 'utf8' }),
				{ fallbackValue: [] },
			);
		} catch (e) {
