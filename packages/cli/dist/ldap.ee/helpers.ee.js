...
const getLdapIds = async () => {
	const identities = await di_1.Container.get(
		...
		return await Promise.all([
			...
			...toDisableUsers.map(async (ldapId) => {
				...
				if (authIdentity?.userId) {
					...
					await transactionManager.delete(auth_identity_1.AuthIdentity, {
...
const createLdapAuthIdentity = async (user, ldapId) => {
	return await di_1.Container.get(auth_identity_repository_1.AuthIdentityRepository).save(
		auth_identity_1.AuthIdentity.create(user, ldapId),
		{
			transaction: false,
		},
	);
...
const updateLdapUserOnLocalDb = async (identity, data) => {
	...
	if (userId) {
		const user = await di_1.Container.get(user_repository_1.UserRepository).findOneBy({
			id: userId,
		});
		if (user) {
...
const deleteAllLdapIdentities = async () => {
	return await di_1.Container.get(auth_identity_repository_1.AuthIdentityRepository).delete({
		providerType: 'ldap',
	});
};
