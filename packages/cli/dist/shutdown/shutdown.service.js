...
let ShutdownService = class ShutdownService {
	...
	register(priority, handler) {
		if (
			priority < constants_1.LOWEST_SHUTDOWN_PRIORITY ||
			priority > constants_1.HIGHEST_SHUTDOWN_PRIORITY
		) {
			throw new n8n_workflow_1.UserError(
				`Invalid shutdown priority. Please set it between ${constants_1.LOWEST_SHUTDOWN_PRIORITY} and ${constants_1.HIGHEST_SHUTDOWN_PRIORITY}.`,
				{ extra: { priority } },
			);
		}
...
exports.ShutdownService = ShutdownService = __decorate(
	[
		(0, di_1.Service)(),
		__metadata('design:paramtypes', [n8n_core_2.Logger, n8n_core_1.ErrorReporter]),
	],
	ShutdownService,
);
