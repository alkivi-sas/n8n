...
let ActiveWorkflowManager = class ActiveWorkflowManager {
	...
	async activateWorkflow(workflowId, activationMode) {
		...
		try {
			...
			if (wasActivated) {
				...
				this.logger.debug(
					`Successfully started workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)}`,
					{
			...
			this.logger.info(
				`     => ERROR: Workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)} could not be activated on first try, keep on trying if not an auth issue`,
			);
	...
	async add(
		...
		try {
			...
			if (shouldDisplayActivationMessage) {
				this.logger.debug(
					`Initializing active workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)} (startup)`,
					{
						workflowName: dbWorkflow.name,
						workflowId: dbWorkflow.id,
					},
	...
	async addTriggersAndPollers(
		...
		if (workflow.getTriggerNodes().length !== 0 || workflow.getPollNodes().length !== 0) {
			this.logger.debug(
				`Adding triggers and pollers for workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)}`,
			);
			...
			this.logger.debug(
				`Workflow ${(0, workflow_formatter_1.formatWorkflow)(dbWorkflow)} activated`,
				{
					workflowId: dbWorkflow.id,
					workflowName: dbWorkflow.name,
				},
