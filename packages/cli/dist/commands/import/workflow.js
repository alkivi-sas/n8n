...
class ImportWorkflowsCommand extends base_command_1.BaseCommand {
	...
	async getWorkflowOwner(workflowId) {
		const sharing = await di_1.Container.get(
	...
	async workflowExists(workflowId) {
		return await di_1.Container.get(workflow_repository_1.WorkflowRepository).existsBy({
			id: workflowId,
		});
	}
	...
	async getProject(userId, projectId) {
		if (projectId) {
			return await di_1.Container.get(project_repository_1.ProjectRepository).findOneByOrFail({
				id: projectId,
			});
		}
		if (!userId) {
		...
		).getPersonalProjectForUserOrFail(userId);
	}
}
