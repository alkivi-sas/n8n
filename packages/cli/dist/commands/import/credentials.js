...
class ImportCredentialsCommand extends base_command_1.BaseCommand {
	...
	async storeCredential(credential, project) {
		const result = await this.transactionManager.upsert(
			credentials_entity_1.CredentialsEntity,
			credential,
			['id'],
		);
		const sharingExists = await this.transactionManager.existsBy(
			shared_credentials_1.SharedCredentials,
			{
	...
	async getCredentialOwner(credentialsId) {
		const sharedCredential = await this.transactionManager.findOne(
			shared_credentials_1.SharedCredentials,
			{
	...
	async credentialExists(credentialId) {
		return await this.transactionManager.existsBy(credentials_entity_1.CredentialsEntity, {
			id: credentialId,
		});
	}
	async getProject(userId, projectId) {
		...
		).getPersonalProjectForUserOrFail(userId, this.transactionManager);
	}
}
