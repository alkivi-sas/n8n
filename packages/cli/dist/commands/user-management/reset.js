...
class Reset extends base_command_1.BaseCommand {
	async run() {
		...
		).makeOwnerOfAllWorkflows(personalProject);
		await di_1.Container.get(
			shared_credentials_repository_1.SharedCredentialsRepository,
		).makeOwnerOfAllCredentials(personalProject);
		await di_1.Container.get(user_repository_1.UserRepository).deleteAllExcept(owner);
		await di_1.Container.get(user_repository_1.UserRepository).save(
			Object.assign(owner, defaultUserProps),
		);
		const danglingCredentials = await di_1.Container.get(
		...
		const newSharedCredentials = danglingCredentials.map((credentials) =>
			di_1.Container.get(shared_credentials_repository_1.SharedCredentialsRepository).create({
				credentials,
				projectId: personalProject.id,
				role: 'credential:owner',
			}),
		);
		await di_1.Container.get(shared_credentials_repository_1.SharedCredentialsRepository).save(
			newSharedCredentials,
		);
		await di_1.Container.get(settings_repository_1.SettingsRepository).update(
	...
	async getInstanceOwner() {
		...
		return await di_1.Container.get(user_repository_1.UserRepository).findOneByOrFail({
			role: 'global:owner',
		});
	}
	async catch(error) {
