...
class Reset extends base_command_1.BaseCommand {
	async run() {
		...
		const ldapIdentities = await di_1.Container.get(
			auth_identity_repository_1.AuthIdentityRepository,
		).find({
		...
		if (flags.projectId ?? flags.userId) {
			...
			await di_1.Container.get(user_repository_1.UserRepository).manager.transaction(
				async (trx) => {
		...
		await di_1.Container.get(
			auth_provider_sync_history_repository_1.AuthProviderSyncHistoryRepository,
		).delete({ providerType: 'ldap' });
		await di_1.Container.get(auth_identity_repository_1.AuthIdentityRepository).delete({
			providerType: 'ldap',
		});
		await di_1.Container.get(user_repository_1.UserRepository).deleteMany(
			ldapIdentities.map((i) => i.userId),
		);
		await di_1.Container.get(project_repository_1.ProjectRepository).delete({
	...
	async getProject(userId, projectId) {
		if (projectId) {
			const project = await di_1.Container.get(project_repository_1.ProjectRepository).findOneBy({
				id: projectId,
			});
			if (project === null) {
		...
		if (userId) {
			...
			).getPersonalProjectForUser(userId);
			if (project === null) {
				throw new n8n_workflow_1.UserError(
	...
	async getOwner() {
		const owner = await di_1.Container.get(user_repository_1.UserRepository).findOneBy({
			role: 'global:owner',
		});
		if (!owner) {
