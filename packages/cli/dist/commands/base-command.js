...
class BaseCommand extends core_1.Command {
	...
	async loadModules() {
		for (const moduleName of this.modulesConfig.modules) {
			await Promise.resolve(`${`../modules/${moduleName}/${moduleName}.module`}`).then((s) =>
	...
	async init() {
		...
		if (communityPackages.enabled && this.needsCommunityPackages) {
			const { CommunityPackagesService } = await Promise.resolve().then(() =>
				__importStar(require('../services/community-packages.service')),
			);
	...
	async initObjectStoreService() {
		...
		const isLicensed = di_1.Container.get(license_1.License).isFeatureEnabled(
	...
	async finally(error) {
		...
		if (constants_1.inTest || this.id === 'start') return;
