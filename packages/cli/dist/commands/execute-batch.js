...
class ExecuteBatch extends base_command_1.BaseCommand {
	...
	async run() {
		...
		ExecuteBatch.instanceOwner = await di_1.Container.get(
			ownership_service_1.OwnershipService,
		).getInstanceOwner();
		const query = di_1.Container.get(workflow_repository_1.WorkflowRepository).createQueryBuilder(
			'workflows',
		);
		if (ids.length > 0) {
				...
				if (data === undefined) {
					...
					if (resultError) {
						...
						if (ExecuteBatch.compare === undefined) {
							...
							const fileName = `${
								ExecuteBatch.compare.endsWith(path_1.sep)
									? ExecuteBatch.compare
									: ExecuteBatch.compare + path_1.sep
						...
						if (ExecuteBatch.snapshot !== undefined) {
							const fileName = `${
								ExecuteBatch.snapshot.endsWith(path_1.sep)
									? ExecuteBatch.snapshot
									: ExecuteBatch.snapshot + path_1.sep
							}${workflowData.id}-snapshot.json`;
