...
export declare class WorkflowRepository extends Repository<WorkflowEntity> {
	...
	getWorkflowsAndFoldersWithCount(
		workflowIds: string[],
		options?: ListQuery.Options,
	): Promise<readonly [WorkflowFolderUnionFull[], number]>;
	...
	getMany(
		...
		(Pick<WorkflowEntity, 'id'> &
			Partial<
				Pick<WorkflowEntity, 'tags' | 'createdAt' | 'updatedAt' | 'name' | 'versionId' | 'active'>
			>)[]
	>;
	getManyAndCount(
		...
		workflows:
			| (Pick<WorkflowEntity, 'id'> &
					Partial<
						Pick<
							WorkflowEntity,
							'tags' | 'createdAt' | 'updatedAt' | 'name' | 'versionId' | 'active'
						>
					>)[]
			| ListQuery.Workflow.WithSharing[];
		count: number;
	...
	findStartingWith(workflowName: string): Promise<
		Array<{
			name: string;
		}>
	>;
