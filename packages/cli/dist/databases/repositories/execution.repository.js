...
function parseFiltersToQueryBuilder(qb, filters) {
	...
	if (filters?.startedAfter) {
		qb.andWhere({
			startedAt: (0, typeorm_1.MoreThanOrEqual)(
				DateUtils_1.DateUtils.mixedDateToUtcDatetimeString(new Date(filters.startedAfter)),
			),
		});
	}
	if (filters?.startedBefore) {
		qb.andWhere({
			startedAt: (0, typeorm_1.LessThanOrEqual)(
				DateUtils_1.DateUtils.mixedDateToUtcDatetimeString(new Date(filters.startedBefore)),
			),
		});
	}
...
const lessThanOrEqual = (date) => {
	return (0, typeorm_1.LessThanOrEqual)(
		DateUtils_1.DateUtils.mixedDateToUtcDatetimeString(new Date(date)),
	);
};
const moreThanOrEqual = (date) => {
	return (0, typeorm_1.MoreThanOrEqual)(
		DateUtils_1.DateUtils.mixedDateToUtcDatetimeString(new Date(date)),
	);
};
let ExecutionRepository = class ExecutionRepository extends typeorm_1.Repository {
	...
	reportInvalidExecutions(executions) {
		...
		this.errorReporter.error(
			new n8n_workflow_1.UnexpectedError('Found executions without executionData', {
	...
	async createNewExecution(execution) {
		...
		if (dbType === 'sqlite' && sqliteConfig.poolSize === 0) {
			...
			return await this.manager.transaction(async (transactionManager) => {
				const { identifiers: inserted } = await transactionManager.insert(
	...
	async markAsCrashed(executionIds) {
		...
		await this.update(
			{ id: (0, typeorm_1.In)(executionIds) },
			{
				status: 'crashed',
				stoppedAt: new Date(),
			},
	...
	async updateExistingExecution(executionId, execution) {
		...
		if (data) executionData.data = (0, flatted_1.stringify)(data);
		const { type: dbType, sqlite: sqliteConfig } = this.globalConfig.database;
		if (dbType === 'sqlite' && sqliteConfig.poolSize === 0) {
	...
	async getIdsSince(date) {
		return await this.find({
			...
			where: {
				startedAt: (0, typeorm_1.MoreThanOrEqual)(
					DateUtils_1.DateUtils.mixedDateToUtcDatetimeString(date),
				),
			},
	...
	async softDeletePrunableExecutions() {
		...
		const toPrune = [
			{
				stoppedAt: (0, typeorm_1.LessThanOrEqual)(
					DateUtils_1.DateUtils.mixedDateToUtcDatetimeString(date),
				),
			},
		];
	...
	async findSoftDeletedExecutions() {
		...
		const workflowIdsAndExecutionIds = (
			await this.find({
				...
				where: {
					deletedAt: (0, typeorm_1.LessThanOrEqual)(
						DateUtils_1.DateUtils.mixedDateToUtcDatetimeString(date),
					),
				},
	...
	async getWaitingExecutions() {
		...
		if (dbType === 'sqlite') {
			where.waitTill = (0, typeorm_1.LessThanOrEqual)(
				DateUtils_1.DateUtils.mixedDateToUtcDatetimeString(waitTill),
			);
		}
	...
	async getExecutionsCountForPublicApi(data) {
		const executions = await this.count({
			where: {
				...
				...(data.excludedWorkflowIds && {
					workflowId: (0, typeorm_1.Not)((0, typeorm_1.In)(data.excludedWorkflowIds)),
				}),
			},
			take: data.limit,
		});
	...
	async getExecutionsForPublicApi(params) {
		...
		if (params.lastId && params.excludedExecutionsIds?.length) {
			where.id = (0, typeorm_1.Raw)(
				(id) => `${id} < :lastId AND ${id} NOT IN (:...excludedExecutionsIds)`,
				{
	...
	toQueryBuilder(query) {
		...
		if (metadata?.length === 1) {
			...
			qb.innerJoin(
				execution_metadata_1.ExecutionMetadata,
				'md',
		...
		if (annotationTags?.length || vote) {
			...
			if (annotationTags?.length) {
				for (let index = 0; index < annotationTags.length; index++) {
					qb.innerJoin(
						annotation_tag_mapping_ee_1.AnnotationTagMapping,
						`atm_${index}`,
	...
	toQueryBuilderWithAnnotations(query) {
		...
		return this.manager
			...
			.leftJoin(
				annotation_tag_mapping_ee_1.AnnotationTagMapping,
				'atm',
				'atm.annotationId = e.annotation_id',
			)
