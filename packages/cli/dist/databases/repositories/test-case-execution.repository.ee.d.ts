...
export declare class TestCaseExecutionRepository extends Repository<TestCaseExecution> {
	...
	markAsRunning({
		...
		executionId,
		trx,
	}: MarkAsRunningOptions): Promise<import('@n8n/typeorm').UpdateResult>;
	markAsEvaluationRunning({
		testRunId,
		pastExecutionId,
		evaluationExecutionId,
		trx,
	}: MarkAsEvaluationRunningOptions): Promise<import('@n8n/typeorm').UpdateResult>;
	markAsCompleted({
		testRunId,
		pastExecutionId,
		metrics,
		trx,
	}: MarkAsCompletedOptions): Promise<import('@n8n/typeorm').UpdateResult>;
	markAllPendingAsCancelled(
		testRunId: string,
		trx?: EntityManager,
	...
	markAsFailed({
		...
		errorDetails,
		trx,
	}: MarkAsFailedOptions): Promise<import('@n8n/typeorm').UpdateResult>;
	markAsWarning({
		testRunId,
		pastExecutionId,
		errorCode,
		errorDetails,
	}: MarkAsWarningOptions): Promise<import('@n8n/typeorm').UpdateResult>;
}
export {};
