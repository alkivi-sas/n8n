...
const getCommonOptions = () => {
	const { tablePrefix: entityPrefix, logging: loggingConfig } = di_1.Container.get(
...
const getSqliteConnectionOptions = () => {
	...
	const commonOptions = {
		...getCommonOptions(),
		database: path_1.default.resolve(
			di_1.Container.get(n8n_core_1.InstanceSettings).n8nFolder,
...
function getConnectionOptions() {
	...
	switch (dbType) {
		...
		default:
			throw new n8n_workflow_1.UserError('Database type currently not supported', {
				extra: { dbType },
			});
	}
}
