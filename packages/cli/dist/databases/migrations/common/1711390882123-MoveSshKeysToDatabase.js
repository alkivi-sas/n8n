...
class MoveSshKeysToDatabase1711390882123 {
	constructor() {
		...
		this.privateKeyPath = node_path_1.default.join(
	...
	async up({ escape, runQuery, logger, migrationName }) {
		...
		try {
			await Promise.all([
				(0, promises_1.rm)(this.privateKeyPath),
				(0, promises_1.rm)(this.publicKeyPath),
			]);
		} catch (e) {
	...
	async down({ escape, runQuery, logger, migrationName }) {
		...
		try {
			await Promise.all([
				(0, promises_1.writeFile)(this.privateKeyPath, privateKey, {
					encoding: 'utf8',
					mode: 0o600,
				}),
