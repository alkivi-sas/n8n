...
function getSqliteDbFileSize() {
	const filename = path_1.default.resolve(
		di_1.Container.get(n8n_core_1.InstanceSettings).n8nFolder,
		di_1.Container.get(config_1.GlobalConfig).database.sqlite.database,
	);
...
const pruneExecutionsData = async ({ queryRunner, tablePrefix, logger }) => {
	if (migrationsPruningEnabled) {
		...
		if (rowCount > 0) {
			...
			const idToKeep = await queryRunner.query(query).then((rows) => rows[0].id);
