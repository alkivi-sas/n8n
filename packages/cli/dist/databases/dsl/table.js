...
class CreateTable extends TableOperation {
	...
	get withTimestamps() {
		this.columns.push(
			new column_1.Column('createdAt').timestamp().notNull.default('NOW()'),
			new column_1.Column('updatedAt').timestamp().notNull.default('NOW()'),
		);
		return this;
	}
...
class AddColumns extends TableOperation {
	...
	async execute(queryRunner) {
		...
		return await queryRunner.addColumns(
			`${prefix}${tableName}`,
			columns.map((c) => new typeorm_1.TableColumn(c.toOptions(driver))),
		);
	}
}
