...
module.exports = {
	...
	transferWorkflow: [
		...
		async (req, res) => {
			...
			await di_1.Container.get(workflow_service_ee_1.EnterpriseWorkflowService).transferOne(
				req.user,
				workflowId,
	...
	getWorkflows: [
		...
		async (req, res) => {
			...
			if (['global:owner', 'global:admin'].includes(req.user.role)) {
				if (tags) {
					const workflowIds = await di_1.Container.get(
						tag_repository_1.TagRepository,
					).getWorkflowIdsViaTags((0, workflows_service_1.parseTagNames)(tags));
				...
				const options = {};
				if (tags) {
					options.workflowIds = await di_1.Container.get(
						tag_repository_1.TagRepository,
					).getWorkflowIdsViaTags((0, workflows_service_1.parseTagNames)(tags));
				}
			...
			const [workflows, count] = await di_1.Container.get(
