...
async function updateWorkflow(workflowId, updateData) {
	return await di_1.Container.get(workflow_repository_1.WorkflowRepository).update(
		workflowId,
		updateData,
	);
}
...
async function updateTags(workflowId, newTags) {
	await Db.transaction(async (transactionManager) => {
		const oldTags = await transactionManager.findBy(workflow_tag_mapping_1.WorkflowTagMapping, {
			workflowId,
		});
		if (oldTags.length > 0) {
		...
		await transactionManager.insert(
			workflow_tag_mapping_1.WorkflowTagMapping,
