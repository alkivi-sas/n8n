...
async function getCredentials(credentialId) {
	return await di_1.Container.get(credentials_repository_1.CredentialsRepository).findOneBy({
		id: credentialId,
	});
}
...
async function saveCredential(credential, user, encryptedData) {
	const result = await Db.transaction(async (transactionManager) => {
		...
		).getPersonalProjectForUserOrFail(user.id, transactionManager);
...
async function removeCredential(user, credentials) {
	...
	return await di_1.Container.get(credentials_repository_1.CredentialsRepository).remove(
		credentials,
	);
}
