...
let OrchestrationService = class OrchestrationService {
	...
	async init() {
		...
		if (config_2.default.get('executions.mode') === 'queue') {
			...
			this.publisher = di_1.Container.get(Publisher);
			const { Subscriber } = await Promise.resolve().then(() =>
				__importStar(require('../scaling/pubsub/subscriber.service')),
			);
			this.subscriber = di_1.Container.get(Subscriber);
		}
