...
let FrontendService = class FrontendService {
	constructor(
		...
		if (this.globalConfig.nodes.communityPackages.enabled) {
			void Promise.resolve()
				.then(() => __importStar(require('../services/community-packages.service')))
	...
	initSettings() {
		...
		this.settings = {
			...
			personalizationSurveyEnabled:
				config_2.default.getEnv('personalization.enabled') && this.globalConfig.diagnostics.enabled,
	...
	getSettings() {
		...
		const isS3Available = config_2.default
			.getEnv('binaryDataManager.availableModes')
			.includes('s3');
		...
		Object.assign(this.settings.enterprise, {
			...
			showNonProdBanner: this.license.isFeatureEnabled(
				constants_1.LICENSE_FEATURES.SHOW_NON_PROD_BANNER,
			),
			debugInEditor: this.license.isDebugInEditorLicensed(),
			binaryDataS3: isS3Available && isS3Selected && isS3Licensed,
			workflowHistory:
				this.license.isWorkflowHistoryLicensed() &&
				config_2.default.getEnv('workflowHistory.enabled'),
