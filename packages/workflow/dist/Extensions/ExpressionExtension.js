...
exports.extendTransform =
	exports.hasNativeMethod =
	exports.hasExpressionExtension =
	...
	try {
		...
		(0, recast_1.visit)(ast, {
			visitChainExpression(path) {
				...
				currentChain += 1;
				const globalIdentifier = recast_1.types.builders.identifier(
					typeof window !== 'object' ? 'global' : 'window',
				);
				const undefinedIdentifier = recast_1.types.builders.identifier('undefined');
				const cancelIdentifier = recast_1.types.builders.identifier(
					`chainCancelToken${chainNumber}`,
				);
				const valueIdentifier = recast_1.types.builders.identifier(`chainValue${chainNumber}`);
				const cancelMemberExpression = recast_1.types.builders.memberExpression(
					globalIdentifier,
					cancelIdentifier,
				);
				const valueMemberExpression = recast_1.types.builders.memberExpression(
					globalIdentifier,
					valueIdentifier,
				);
				const patchedStack = [];
				const buildCancelCheckWrapper = (node) => {
					return recast_1.types.builders.conditionalExpression(
						recast_1.types.builders.binaryExpression(
							'===',
							cancelMemberExpression,
							recast_1.types.builders.booleanLiteral(true),
						),
						undefinedIdentifier,
						node,
					);
				...
				const buildOptionalWrapper = (node) => {
					return recast_1.types.builders.binaryExpression(
						'===',
						recast_1.types.builders.logicalExpression(
							'??',
				...
				while (true) {
					if (
						...
						if (
							...
							patchNode = recast_1.types.builders.memberExpression(
								valueMemberExpression,
								currentNode.property,
							);
						} else {
							patchNode = recast_1.types.builders.callExpression(
								valueMemberExpression,
						...
						if (currentNode.optional) {
							if (wrapNextTopInOptionalExtend) {
								...
								if (
									...
									patchTop = recast_1.types.builders.callExpression(
										recast_1.types.builders.identifier(EXPRESSION_EXTENDER_OPTIONAL),
										[
											patchTop.object,
											recast_1.types.builders.stringLiteral(patchTop.property.name),
										],
									);
								}
							}
						...
						if (wrapNextTopInOptionalExtend) {
							...
							if (
								...
								patchTop = recast_1.types.builders.callExpression(
									recast_1.types.builders.identifier(EXPRESSION_EXTENDER_OPTIONAL),
									[patchTop.object, recast_1.types.builders.stringLiteral(patchTop.property.name)],
								);
							}
						}
						if (patchTop) {
		...
		(0, recast_1.visit)(ast, {
			visitCallExpression(path) {
				...
				if (
					...
					path.replace(
						recast_1.types.builders.callExpression(
							recast_1.types.builders.identifier(EXPRESSION_EXTENDER),
							[
					...
					if (path.node.arguments.length < 2) {
						throw new expression_extension_error_1.ExpressionExtensionError(
					...
					path.replace(
						recast_1.types.builders.conditionalExpression(test, consequent, alternative),
					);
				}
			},
		});
...
function extendSyntax(bracketedExpression, forceExtend = false) {
	...
	if (
		(!codeChunks.some(exports.hasExpressionExtension) ||
			(0, exports.hasNativeMethod)(bracketedExpression)) &&
		!forceExtend
	) {
