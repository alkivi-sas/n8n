...
class Workflow {
	...
	getStaticData(type, node) {
		...
		if (type === 'global') {
			...
			throw new application_error_1.ApplicationError(
				'Unknown context type. Only `global` and `node` are supported.',
				{
		...
		if (typeof parameterValue !== 'object') {
			if (
				...
				if (parameterValue.includes(currentName)) {
					...
					const setNewName = (expression, oldPattern) =>
	...
	getHighestNode(nodeName, nodeConnectionIndex, checkedNodes) {
		...
		if (
			!this.connectionsByDestinationNode[nodeName].hasOwnProperty(
		...
		for (
			...
			connectionIndex <
			this.connectionsByDestinationNode[nodeName][Interfaces_1.NodeConnectionTypes.Main].length;
			connectionIndex++
		) {
			...
			connectionsByIndex =
				this.connectionsByDestinationNode[nodeName][Interfaces_1.NodeConnectionTypes.Main][
					connectionIndex
				];
	...
	getConnectedNodes(
		connections,
		nodeName,
		connectionType = Interfaces_1.NodeConnectionTypes.Main,
		depth = -1,
	...
	getParentMainInputNode(node) {
		if (node) {
			...
			if (
				!!outputs.find(
					(output) => (output?.type ?? output) !== Interfaces_1.NodeConnectionTypes.Main,
				)
			) {
				...
				if (nonMainNodesConnected.length) {
					...
					if (returnNode === null) {
						throw new application_error_1.ApplicationError(
							`Node "${nonMainNodesConnected[0]}" not found`,
						);
					}
	...
	__getStartNode(nodeNames) {
		...
		const sortedNodeNames = Object.values(this.nodes)
			.sort(
				(a, b) =>
					Constants_1.STARTING_NODE_TYPES.indexOf(a.type) -
					Constants_1.STARTING_NODE_TYPES.indexOf(b.type),
			)
			.map((n) => n.name);
