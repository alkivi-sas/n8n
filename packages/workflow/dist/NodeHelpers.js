			...
			for (const itemName of Object.keys(propertyValues || {})) {
				if (
					...
					if (nodePropertyOptions !== undefined) {
						...
						tempValue = getNodeParameters(
							tempNodePropertiesArray,
							nodeValues[nodeProperties.name][itemName],
							returnDefaults,
							returnNoneDisplayed,
							node,
							nodeTypeDescription,
...
function getNodeOutputs(workflow, node, nodeTypeData) {
	...
	if (Array.isArray(nodeTypeData.outputs)) {
		...
		try {
			outputs =
				workflow.expression.getSimpleParameterValue(node, nodeTypeData.outputs, 'internal', {}) ||
				[];
		} catch (e) {
...
const validateResourceMapperParameter = (nodeProperties, value, skipRequiredCheck = false) => {
	...
	value.schema.forEach((field) => {
		...
		if (!fieldValue?.toString().startsWith('=') && field.type) {
			const validationResult = (0, TypeValidation_1.validateFieldType)(
				field.id,
				fieldValue,
				field.type,
				{
