...
class WorkflowDataProxy {
	constructor(
		...
		this.connectionInputData = isScriptingNode(this.contextNodeName, workflow)
			...
			: connectionInputData;
		this.timezone =
			workflow.settings?.timezone ?? (0, GlobalState_1.getGlobalState)().defaultTimezone;
	...
	nodeContextGetter(nodeName) {
		...
		if (!that.runExecutionData?.executionData && !that.runExecutionData?.resultData) {
			throw new expression_error_1.ExpressionError(
	...
	nodeParameterGetter(nodeName, resolveValue = true) {
		...
		return new Proxy(node?.parameters ?? {}, {
			...
			get(target, name) {
				if (name === 'isProxy') return true;
				if (name === 'toJSON') return () => (0, utils_1.deepCopy)(target);
				name = name.toString();
				let returnValue;
				if (name[0] === '&') {
					const key = name.slice(1);
					if (!that.siblingParameters.hasOwnProperty(key)) {
						throw new application_error_1.ApplicationError(
	...
	getNodeExecutionOrPinnedData({ nodeName, branchIndex, runIndex, shortSyntax = false }) {
		try {
			...
			const pinData = (0, WorkflowDataProxyHelpers_1.getPinDataIfManualExecution)(
				this.workflow,
				nodeName,
				this.mode,
			);
	...
	getNodeExecutionData(nodeName, shortSyntax = false, outputIndex, runIndex) {
		...
		if (!shortSyntax) {
			...
			if (
				...
				!(0, WorkflowDataProxyHelpers_1.getPinDataIfManualExecution)(
					that.workflow,
					nodeName,
					that.mode,
				)
	...
	getDataProxy(opts) {
			...
			while (sourceData !== null && destinationNodeName !== sourceData.previousNode) {
				...
				if (itemInput >= source.length) {
					...
					throw createExpressionError('Can’t get data for expression', {
						...
						nodeCause: nodeBeforeLast,
						description: `In node ‘<strong>${sourceData.previousNode}</strong>’, output item ${currentPairedItem.item || 0} of ${
		...
		const handleFromAi = (name, _description, _type = 'string', defaultValue) => {
			...
			const placeholdersDataInputData =
				inputData?.[Interfaces_1.NodeConnectionTypes.AiTool]?.[0]?.[itemIndex].json;
		...
		const base = {
							...
							if (property === 'pairedItem' || property === 'itemMatching' || property === 'item') {
								...
								const pairedItemMethod = (itemIndex) => {
									...
									if (!that.connectionInputData.length) {
										const pinnedData = (0, WorkflowDataProxyHelpers_1.getPinDataIfManualExecution)(
											that.workflow,
											nodeName,
											that.mode,
										);
			...
			$binary: {},
			$data: {},
			$env: (0, WorkflowDataProxyEnvProvider_1.createEnvProvider)(
				that.runIndex,
				that.itemIndex,
				that.envProviderState ?? (0, WorkflowDataProxyEnvProvider_1.createEnvProviderState)(),
			),
