...
function getDomainPath(raw, urlParts = URL_PARTS_REGEX) {
	try {
		const url = new URL(raw);
		if (!url.hostname) throw new application_error_1.ApplicationError('Malformed URL');
...
function generateNodesGraph(workflow, nodeTypes, options) {
	...
	const otherNodes = (workflow.nodes ?? []).filter(
		(node) => node.type !== Constants_1.STICKY_NODE_TYPE,
	);
	notes.forEach((stickyNote, index) => {
		const stickyType = nodeTypes.getByNameAndVersion(
			Constants_1.STICKY_NODE_TYPE,
			stickyNote.typeVersion,
		);
		if (!stickyType) {
			return;
		}
		...
		try {
			nodeParameters =
				(0, NodeHelpers_1.getNodeParameters)(
	...
	otherNodes.forEach((node, index) => {
		...
		if (node.type === Constants_1.AI_TRANSFORM_NODE_TYPE && options?.isCloudDeployment) {
			...
			try {
				...
				if (nodeType) {
					const nodeParameters = (0, NodeHelpers_1.getNodeParameters)(
					...
					if (nodeParameters) {
						const keys = ['operation', 'resource', 'mode'];
						keys.forEach((key) => {
		...
		if (options?.isCloudDeployment === true) {
			if (node.type === Constants_1.OPENAI_LANGCHAIN_NODE_TYPE) {
				nodeItem.prompts = (node.parameters?.messages ?? {}).values ?? [];
			}
			...
			if (node.type === Constants_1.CHAIN_SUMMARIZATION_LANGCHAIN_NODE_TYPE) {
				nodeItem.prompts = (
					(node.parameters?.options ?? {}).summarizationMethodAndPrompts ?? {}
				).values;
			}
			...
			if (node.type === Constants_1.CHAIN_LLM_LANGCHAIN_NODE_TYPE) {
				nodeItem.prompts = (node.parameters?.messages ?? {}).messageValues ?? [];
			}
			if (
				node.type === Constants_1.MERGE_NODE_TYPE &&
				node.parameters?.operation === 'combineBySql'
			) {
				nodeItem.sql = node.parameters?.query;
			}
		}
...
const userInInstanceRanOutOfFreeAiCredits = (runData) => {
	...
	if (credentials.credentialType !== Constants_1.OPEN_AI_API_CREDENTIAL_TYPE) return false;
	...
	try {
		...
		if (
			errorResponse?.error?.type === Constants_1.FREE_AI_CREDITS_ERROR_TYPE &&
...
function resolveAIMetrics(nodes, nodeTypes) {
	...
	for (const [node, _] of tools) {
		...
		const overrides = values.reduce(
			(acc, value) =>
				acc + Number(value.startsWith(`={{ ${Constants_1.FROM_AI_AUTO_GENERATED_MARKER} $fromA`)),
			0,
		);
