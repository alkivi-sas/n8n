		...
		visitMemberExpression(path) {
			...
			if (!node.computed) {
				if (node.property.type !== 'Identifier') {
					throw new errors_1.ExpressionError(
				...
				path.replace(
					tournament_1.astBuilders.memberExpression(
						node.object,
						tournament_1.astBuilders.callExpression(
							tournament_1.astBuilders.memberExpression(dataNode, sanitizerIdentifier),
							[node.property],
						),
						true,
					),
