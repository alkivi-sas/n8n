...
const isExpressionError = (error) =>
	error instanceof expression_error_1.ExpressionError ||
	error instanceof expression_extension_error_1.ExpressionExtensionError;
...
class Expression {
	...
	convertObjectValueToString(value) {
		...
		if (value instanceof Date) {
			result = luxon_1.DateTime.fromJSDate(value, {
				zone:
					this.workflow.settings?.timezone ?? (0, GlobalState_1.getGlobalState)().defaultTimezone,
			}).toISO();
	...
	resolveSimpleParameterValue(
		...
		const dataProxy = new WorkflowDataProxy_1.WorkflowDataProxy(
			this.workflow,
			runExecutionData,
			runIndex,
			itemIndex,
	...
	renderExpression(expression, data) {
		try {
			...
			if (isExpressionError(error)) throw error;
			if (isSyntaxError(error)) throw new application_error_1.ApplicationError('invalid syntax');
