...
class NodeApiError extends node_error_1.NodeError {
	constructor(
		...
		if (!description && (errorResponse.description || errorResponse?.reason?.description)) {
			this.description = errorResponse.description || errorResponse?.reason?.description;
		}
		if (!message && (errorResponse.message || errorResponse?.reason?.message || description)) {
			this.message = errorResponse.message || errorResponse?.reason?.message || description;
		}
		...
		[this.message, this.messages] = this.setDescriptiveErrorMessage(
			this.message,
			this.messages,
			this.httpCode || errorResponse?.code || errorResponse?.reason?.code || undefined,
			messageMapping,
		);
	...
	setDefaultStatusCodeMessage() {
		...
		if (
			this.node.type === Constants_1.NO_OP_NODE_TYPE &&
			this.message === Constants_1.UNKNOWN_ERROR_MESSAGE
		) {
